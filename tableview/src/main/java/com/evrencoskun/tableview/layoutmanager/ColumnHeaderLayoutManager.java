...
public class ColumnHeaderLayoutManager extends LinearLayoutManager
{
  ...
  @Override public void measureChildWithMargins(View child, int widthUsed, int heightUsed)
  {
  ...
  @Override public void measureChild(View child, int widthUsed, int heightUsed)
  {
    // If has fixed width is true, than calculation of the column width is not necessary.
    if (mTableView.hasFixedWidth())
    {
  ...
  public void customRequestLayout()
  {
    ...
    for (int i = findFirstVisibleItemPosition(); i < findLastVisibleItemPosition() + 1; i++)
    {
      ...
      layoutDecoratedWithMargins(columnHeader,
                                 columnHeader.getLeft(),
                                 columnHeader.getTop(),
                                 columnHeader.getRight(),
                                 columnHeader.getBottom());
  ...
  public AbstractViewHolder[] getVisibleViewHolders()
  {
    ...
    for (int i = findFirstVisibleItemPosition(); i < findLastVisibleItemPosition() + 1; i++)
    {
      ...
      views[index] = (AbstractViewHolder) mTableView.getColumnHeaderRecyclerView().findViewHolderForAdapterPosition(i);
  ...
  {
    return (AbstractViewHolder) mTableView.getColumnHeaderRecyclerView().findViewHolderForAdapterPosition(xPosition);
  }
